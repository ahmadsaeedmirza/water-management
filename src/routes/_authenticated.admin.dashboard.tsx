...
import { Droplet, DollarSign, Wallet, TrendingDown, Activity, X, Banknote, CreditCard, Globe, Phone, MapPin, Plus } from "lucide-react";
...
function AdminDashboard() {
  ...
  const expensesQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("expenses").select("name, amount, created_at").gte("created_at", since).order("created_at", { ascending: false });
      if (error) throw error;
      return data ?? [];
    },
  });
  ...
  const paymentsQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("payments").select("amount, created_at").gte("created_at", since);
      if (error) throw error;
      return data ?? [];
    },
  });
  ...
  const breakdownQ = useQuery({
    ...
    queryFn: async () => {
      ...
      const [lots, deliveries, expenses] = await Promise.all([
        ...
        supabase.from("deliveries").select("*, customers(name)").gte("created_at", start).lte("created_at", end),
  ...
  return (
    ...
    >
      <div className="sm:hidden mb-5 inline-flex rounded-[10px] bg-muted p-1">
        {(["today", "month"] as Range[]).map((r) => (
          <button key={r} onClick={() => setRange(r)} className={`px-4 h-9 rounded-[8px] text-sm font-semibold capitalize ${range === r ? "bg-card text-primary shadow-sm" : "text-muted-foreground"}`}>
            {r === "today" ? "Today" : "This Month"}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        <Kpi icon={Droplet} tint="bg-accent text-primary" label="Total Bottles Sold" value={bottles.toString()} />
        <Kpi icon={DollarSign} tint="bg-success/10 text-success" label="Total Revenue" value={formatRs(sales)} />
        <Kpi icon={TrendingDown} tint="bg-destructive/10 text-destructive" label="Total Expenses" value={formatRs(expenses)} />
        <Kpi icon={Wallet} tint={net < 0 ? "bg-destructive/10 text-destructive" : "bg-success/10 text-success"} label="Net Revenue" value={formatRs(net)} negative={net < 0} />
      </div>
      ...
      <div className="card-surface p-5 mt-5">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{chartMode === "bottles" ? "Bottles delivered per day" : "Revenue collected per day"}</p>
          </div>
          <div className="flex items-center gap-4">
            ...
            <div className="text-right">
              <p className="text-xs text-muted-foreground">Net ({range === "today" ? "Today" : "Month"})</p>
              <p className={`font-bold tabular-nums ${net >= 0 ? "text-success" : "text-destructive"}`}>{formatRs(net)}</p>
            </div>
          </div>
        </div>
        <div className="h-64 -mx-2 cursor-pointer">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: "var(--color-muted-foreground)" }} width={40} />
              <Tooltip
                cursor={{ fill: "var(--color-muted)" }}
                contentStyle={{ borderRadius: 10, border: "1px solid var(--color-border)", fontSize: 12 }}
                formatter={(v: number, n) => [chartMode === "revenue" ? formatRs(v) : v, chartMode === "revenue" ? "Sales" : "Bottles"]}
              />
              <Bar dataKey={chartMode === "bottles" ? "bottles" : "sales"} radius={[6, 6, 0, 0]}>
                {chartData.map((entry, index) => {
                  const isToday = entry.date === new Date().toISOString().slice(0, 10);
                  return (
                    <Cell key={`cell-${index}`} fill={isToday ? "#00B4D8" : "#0077B6"} />
                  );
                })}
              </Bar>
            </BarChart>
      ...
      <div className="grid md:grid-cols-2 gap-5 mt-5">
        ...
        <div className="card-surface overflow-hidden">
          ...
          {deliveries.length === 0 ? (
            <div className="px-5 py-10 text-center text-sm text-muted-foreground">No deliveries in this range yet.</div>
          ) : (
            <ul className="divide-y divide-border">
              {deliveries.slice(0, 10).map((d: any) => (
                <li key={d.id} className="px-5 py-3.5 flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <p className="font-semibold text-sm truncate">{d.customers?.name ?? "Walk-in"}</p>
                    <p className="text-xs text-muted-foreground">{d.bottles_delivered} bottle(s) · {formatTime(d.created_at)} · {relativeTime(d.created_at)}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="font-bold tabular-nums text-sm">{formatRs(d.total_amount)}</p>
                    <p className={`text-[10px] font-semibold uppercase ${d.payment_mode === "pending" ? "text-warning" : "text-success"}`}>{d.payment_mode}</p>
                  </div>
                </li>
              ))}
            </ul>
        ...
        <div className="card-surface overflow-hidden">
          ...
          {recentExpensesQ.isLoading ? (
            <div className="px-5 py-10 text-center text-sm text-muted-foreground animate-pulse">Loading...</div>
          ) : (recentExpensesQ.data ?? []).length === 0 ? (
            <div className="px-5 py-10 text-center text-sm text-muted-foreground">No expenses logged yet.</div>
          ) : (
            <ul className="divide-y divide-border">
              {(recentExpensesQ.data ?? []).slice(0, 10).map((e: any) => (
                <li key={e.id} className="px-5 py-3.5 flex items-center justify-between gap-3">
                  ...
                  <div className="text-right shrink-0">
                    <p className="font-bold tabular-nums text-sm text-destructive">{formatRs(e.amount)}</p>
                  </div>
                </li>
              ))}
            </ul>
      ...
      <div className="card-surface mt-5 overflow-hidden">
        ...
        {customersSummaryQ.isLoading ? (
          <div className="p-5 text-center text-sm text-muted-foreground animate-pulse">Loading customers summary...</div>
        ) : (customersSummaryQ.data ?? []).length === 0 ? (
          <div className="p-5 text-center text-sm text-muted-foreground">No regular customers registered.</div>
        ) : (
                ...
                {(customersSummaryQ.data ?? []).map((c: any) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4 text-center font-medium tabular-nums">{bottlesThisMonth}</td>
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-md h-full bg-card border-l border-border p-5 flex flex-col animate-in slide-in-from-right duration-250">
            <div className="flex items-center justify-between pb-3 border-b border-border mb-4">
              ...
              <button onClick={() => setSelectedDate(null)} className="h-9 w-9 rounded-[10px] grid place-items-center hover:bg-muted"><X className="h-5 w-5" /></button>
            </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-5 pr-1">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="card-surface p-3 bg-muted/20">
                    <p className="text-[10px] uppercase font-semibold text-muted-foreground">Revenue</p>
                    <p className="text-lg font-bold tabular-nums text-success">
                      {formatRs((breakdownQ.data?.deliveries ?? []).reduce((a: number, d: any) => a + Number(d.total_amount), 0))}
                    </p>
                  </div>
                  <div className="card-surface p-3 bg-muted/20">
                    <p className="text-[10px] uppercase font-semibold text-muted-foreground">Expenses</p>
                    <p className="text-lg font-bold tabular-nums text-destructive">
                      {formatRs((breakdownQ.data?.expenses ?? []).reduce((a: number, e: any) => a + Number(e.amount), 0))}
                    </p>
                  </div>
                </div>
                ...
                {/* Lots section */}
                <div>
                  <h4 className="font-bold text-xs uppercase tracking-wider text-muted-foreground mb-2">Lots Loaded</h4>
                  {(breakdownQ.data?.lots ?? []).length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {(breakdownQ.data?.lots ?? []).map((l: any) => {
                        const sold = (breakdownQ.data?.deliveries ?? []).filter((d: any) => d.lot_id === l.id).reduce((a: number, d: any) => a + d.bottles_delivered, 0);
                        return (
                          <div key={l.id} className="card-surface p-3 text-xs flex justify-between">
                            <div>
                              ...
                              <p className="text-muted-foreground mt-0.5">Taken: {l.total_bottles} · Sold: {sold}</p>
                            </div>
                            <span className="font-bold text-primary">{l.total_bottles - sold} left</span>
                          </div>
                        );
                      })}
                    </div>
                ...
                {/* Deliveries section */}
                <div>
                  <h4 className="font-bold text-xs uppercase tracking-wider text-muted-foreground mb-2">Deliveries</h4>
                  {(breakdownQ.data?.deliveries ?? []).length === 0 ? (
                    <p className="text-xs text-muted-foreground py-2 italic">No deliveries recorded.</p>
                  ) : (
                    <div className="space-y-2">
                      {(breakdownQ.data?.deliveries ?? []).map((d: any) => (
                        <div key={d.id} className="card-surface p-3 text-xs flex justify-between items-center">
                          <div>
                            <p className="font-semibold">{d.customers?.name ?? "Walk-in"}</p>
                            <p className="text-muted-foreground mt-0.5">{d.bottles_delivered} bottle(s) · {formatTime(d.created_at)}</p>
                          </div>
                          <div className="text-right">
                            <p className="font-bold tabular-nums">{formatRs(d.total_amount)}</p>
                            <span className="text-[10px] uppercase font-bold text-muted-foreground">{d.payment_mode}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {/* Expenses section */}
                <div>
                  <h4 className="font-bold text-xs uppercase tracking-wider text-muted-foreground mb-2">Expenses</h4>
                  {(breakdownQ.data?.expenses ?? []).length === 0 ? (
                    <p className="text-xs text-muted-foreground py-2 italic">No expenses recorded.</p>
                  ) : (
                    <div className="space-y-2">
                      {(breakdownQ.data?.expenses ?? []).map((e: any) => (
                        <div key={e.id} className="card-surface p-3 text-xs flex justify-between items-center">
                          <div>
                            <p className="font-semibold">{e.name}</p>
                            <p className="text-muted-foreground mt-0.5">{formatTime(e.created_at)}</p>
                          </div>
                          <span className="font-bold text-destructive tabular-nums">{formatRs(e.amount)}</span>
                        </div>
                      ))}
                    </div>
                  )}
...
function Kpi({ icon: Icon, tint, label, value, negative }: { icon: any; tint: string; label: string; value: string; negative?: boolean }) {
  return (
    <div className="card-surface p-4">
      ...
      <p className={`mt-0.5 text-xl md:text-2xl font-bold tabular-nums truncate ${negative ? "text-destructive" : ""}`}>{value}</p>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-card border-l border-border flex flex-col animate-in slide-in-from-right duration-250">
        ...
        {payOpen && (
          <div className="absolute inset-0 z-10 flex items-end bg-black/40 animate-in fade-in duration-200" onClick={() => setPayOpen(false)}>
